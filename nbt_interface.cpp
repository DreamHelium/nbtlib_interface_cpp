...
static void
parse_nbt_real (DhNbtInstance &instance, NbtNode *nbt)
{
...
static void
parse_nbt_tmp (DhNbtInstance &instance, NbtNode *nbt)
{
...
static void
parse_nbt (DhNbtInstance &instance, NbtNode *nbt, bool tr)
{
    if (tr)
        parse_nbt_tmp (instance, nbt);
    else
        parse_nbt_real (instance, nbt);
}

static bool
has_child (DhNbtInstance parent, DhNbtInstance child)
{
    ...
    if (parent_node->children && child.is_non_null ())
        {
            ...
            for (; child_node; child_node = child_node->next)
                {
                    if (child_node == child.get_current_nbt ())
                        return true;
                }
            return false;
        }
...
DhNbtInstance::DhNbtInstance (const gint8 *val, int len, const char *key,
                              bool temporary_root)
{
    auto new_nbt = ret_non_filled_nbt ();
    auto new_nbt_data = g_new0 (NbtData, 1);
...
DhNbtInstance::DhNbtInstance (const gint32 *val, int len, const char *key,
                              bool temporary_root)
{
    auto new_nbt = ret_non_filled_nbt ();
    auto new_nbt_data = g_new0 (NbtData, 1);
...
DhNbtInstance::DhNbtInstance (const gint64 *val, int len, const char *key,
                              bool temporary_root)
{
    auto new_nbt = ret_non_filled_nbt ();
    auto new_nbt_data = g_new0 (NbtData, 1);
...
DhNbtInstance::DhNbtInstance (DhNbtType type, const char *key,
                              bool temporary_root)
{
    auto new_nbt = ret_non_filled_nbt ();
    auto new_nbt_data = g_new0 (NbtData, 1);
...
bool
DhNbtInstance::is_type (DhNbtType type)
{
    if (!is_non_null ())
        {
...
bool
DhNbtInstance::child (const char *key)
{
    if (child ())
        {
            do
...
void
DhNbtInstance::goto_root ()
{
    current_nbt = get_original_nbt ();
}

const char *
DhNbtInstance::get_key ()
{
    if (is_non_null ())
        {
            NbtData *data = (NbtData *)this->current_nbt->data;
...
void
DhNbtInstance::set_key (const char *key)
{
    if (is_non_null ())
        {
            NbtData *data = (NbtData *)this->current_nbt->data;
...
void
DhNbtInstance::make_invalid ()
{
    current_nbt = nullptr;
}

bool
DhNbtInstance::prepend (DhNbtInstance child)
{
...
bool
DhNbtInstance::insert_after (DhNbtInstance sibling, DhNbtInstance node)
{
    if (has_child (*this, sibling))
        {
            g_node_insert_after (current_nbt, sibling.current_nbt,
                                 node.current_nbt);
            return true;
        }
    else
...
bool
DhNbtInstance::insert_before (DhNbtInstance sibling, DhNbtInstance node)
{
    if (has_child (*this, sibling))
        {
            g_node_insert_before (current_nbt, sibling.current_nbt,
                                  node.current_nbt);
            return true;
        }
    else
...
gint8
DhNbtInstance::get_byte ()
{
    auto data = (NbtData *)this->current_nbt->data;
    if (is_type (DH_TYPE_Byte))
        return data->value_i;
    else
        throw std::domain_error ("Not the right type!");
}

gint16
DhNbtInstance::get_short ()
{
    auto data = (NbtData *)this->current_nbt->data;
    if (is_type (DH_TYPE_Short))
        return data->value_i;
    else
        throw std::domain_error ("Not the right type!");
}
...
gint32
DhNbtInstance::get_int ()
{
    auto data = (NbtData *)this->current_nbt->data;
    if (is_type (DH_TYPE_Int))
        return data->value_i;
    else
        throw std::domain_error ("Not the right type!");
}
...
gint64
DhNbtInstance::get_long ()
{
    auto data = (NbtData *)this->current_nbt->data;
    if (is_type (DH_TYPE_Long))
        return data->value_i;
    else
        throw std::domain_error ("Not the right type!");
}
...
float
DhNbtInstance::get_float ()
{
    auto data = (NbtData *)this->current_nbt->data;
    if (is_type (DH_TYPE_Float))
        return data->value_d;
    else
        throw std::domain_error ("Not the right type!");
}
...
double
DhNbtInstance::get_double ()
{
    auto data = (NbtData *)this->current_nbt->data;
    if (is_type (DH_TYPE_Double))
        return data->value_d;
    else
        throw std::domain_error ("Not the right type!");
}
...
const gchar *
DhNbtInstance::get_string ()
{
    auto data = (NbtData *)this->current_nbt->data;
    if (is_type (DH_TYPE_String))
        return (const char *)data->value_a.value;
    else
        throw std::domain_error ("Not the right type!");
}

const gint8 *
DhNbtInstance::get_byte_array (int &len)
{
    auto data = (NbtData *)this->current_nbt->data;
...
const gint32 *
DhNbtInstance::get_int_array (int &len)
{
    auto data = (NbtData *)this->current_nbt->data;
...
const gint64 *
DhNbtInstance::get_long_array (int &len)
{
    auto data = (NbtData *)this->current_nbt->data;
...
void
DhNbtInstance::set_string (const char *str)
{
    auto data = (NbtData *)this->current_nbt->data;
...
DhNbtInstance
DhNbtInstance::dup_current_as_original (bool temp_root)
{
    if (is_type (DH_TYPE_Byte))
    ...
    else
        {
            ...
            for (; origin_child.is_non_null (); origin_child.next ())
                {
                    DhNbtInstance cur
                        = origin_child.dup_current_as_original (true);
...
static void
rm_node_internal (DhNbtInstance &child)
{
...
bool
DhNbtInstance::rm_node (const char *key)
{
...
bool
DhNbtInstance::rm_node (int index)
{
    DhNbtInstance child (*this);
...
{
    void *
    dh_nbt_instance_cpp_new ()
    {
        return new DhNbtInstance ();
    }

    void
    dh_nbt_instance_cpp_free (void *mem)
    {
        DhNbtInstance *instance
            = dynamic_cast<DhNbtInstance *> ((DhNbtInstance *)mem);
        delete instance;
    }
}